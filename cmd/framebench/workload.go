...
type sender func(context.Context, []votelog.Input) (votelog.FrameReceipt, error)
...
func digest(inputs []votelog.Input) [32]byte {
	h := sha256.New()
	var encoded [20]byte
...
func runWorkload(parent context.Context, c config, cfg votelog.Config, seed [16]byte, states states, send sender) (workloadReport, []ackFrame) {
	...
	for worker := range c.workers {
		wg.Go(func() {
			...
			for f := range jobs {
				...
				callCancel()
				state := stateUnknown
				if err == nil && (receipt.Partition != f.Partition || int(receipt.Count) != len(f.Inputs) || receipt.Offset < 0 || receipt.AdmittedAt.Before(cfg.StartsAt) || !receipt.AdmittedAt.Before(cfg.EndsAt)) {
					s.counts.InvalidReceipts++
					err = votelog.ErrUnknown
				}
				switch {
				case err == nil:
					state = stateACK
	...
	close(jobs)
	wg.Wait()
	r := workloadReport{StartsAt: cfg.StartsAt.UTC(), FinishedAt: time.Now().UTC(), PlannedKeys: c.keys(), Buckets: make([]bucket, 82), LatencyMethod: "Record ACK latency is sampled independently of outcome by low10 AES-token bits (all attempts if planned<=10000); both repeat choices share selection. Histogram quantiles are upper edges with <6.25% rounding. Skipped/unknown/rejected attempts do not enter ACK latency samples. Frame dispatch includes direct Store.SubmitFrame, not external HTTP/TLS."}
	...
	r.FramePayloadBytes = r.Counts.Recorded*28 + r.Counts.RecordedFrames*80
...
func generate(ctx context.Context, c config, cfg votelog.Config, seed [16]byte, jobs chan<- frame) stat {
	var s stat
	cipher, _ := aes.NewCipher(seed[:])
	var counter [16]byte
	var inputBuffer [256]votelog.Input
	buffers := make([]frame, c.partitions)
	for p := range buffers {
		buffers[p].Partition = int32(p)
	}
	newBuffer := func(p int) {
		buffers[p] = frame{Inputs: make([]votelog.Input, 0, c.frameSize), Keys: make([]uint32, 0, c.frameSize), Partition: int32(p)}
	}
	flush := func(p int) {
		f := buffers[p]
		if len(f.Inputs) == 0 {
	...
	for base := uint64(0); base < c.keys(); base += uint64(groupSize) {
		...
		for i := 0; i < n; i++ {
			...
			appendInput := func(input votelog.Input) {
