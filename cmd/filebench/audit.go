...
type replayer func(context.Context, filelog.Config, func(filelog.Position, filelog.Vote) error) (filelog.Manifest, error)
...
func reconcile(ctx context.Context, m manifest, s states, acks []ackFrame, replay replayer) (auditReport, error) {
	...
	flush := func(partition int) error {
		...
		r.RecordedFrames++
		r.PayloadBytes += 40 + 28*uint64(a.next)
	...
	manifest, err := replay(ctx, m.Config, func(pos filelog.Position, vote filelog.Vote) error {
	...
	if err == nil && (len(manifest.Partitions) != 1 || manifest.Partitions[0].Partition != 0 || manifest.Partitions[0].Offset < 0 || r.RecordedFrames > 0 && manifest.Partitions[0].Offset <= acc[0].offset) {
		err = fmt.Errorf("invalid terminal CLOSED position")
	}
