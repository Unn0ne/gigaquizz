...
Обновление от 10.09.2026: приведённая ниже смета 39 узлов снята с рекомендации к развёртыванию. Она сохраняется как пример расчёта по условным параметрам; необходимость такого парка измерениями не подтверждена. [Один Go-процесс и файловый журнал](simplification.md) уже сохранил более 100 млн новых голосов в двух минутных direct-прогонах. Это проверка локального ядра с одной копией данных; полный HTTP-путь и устойчивость к потере хоста ещё требуют испытаний. [Результаты](file-results.md).
