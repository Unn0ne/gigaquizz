...
[Теоретический расчёт железа от 17 сентября](hardware-sizing.md) уточняет объём данных, RAM, CPU и сеть обеих веток. Приведённый ниже профиль остаётся кандидатом для испытаний, а не минимальными требованиями: при меньшем RAM обязательно уменьшите GOMEMLIMIT.
