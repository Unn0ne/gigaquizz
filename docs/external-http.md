...
Для установки постоянной службы используйте [серверный профиль и systemd](server-deployment.md). [Последнее ревью и проверка переноса](server-readiness-review.md) фиксируют фактически проверенные платформы и ограничения.
